...
const addProduct = (e) => {
    ...
    let plushies = document.getElementById("category-plushies").checked;
    let homeDecor = document.getElementById("category-homedecor").checked;
    let accesories = document.getElementById("category-accesories").checked;

    const category = plushies && 'plushies' || homeDecor && 'homeDecor' || accesories && 'accesories';

    const product = { title, code, price, stock, description, thumbnails: [thumbnail], category };
...
const deleteProduct = (pid) => {
    socket.emit('ondeleteproduct', parseInt(pid));
    return false;
}

const createProductHtml = (product) => {
    const { title, code, price, stock, description, thumbnails, category, id } = product;

    return `
    <div class="group relative" id="${id}">
        <div class="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-md bg-gray-200 lg:aspect-none group-hover:opacity-75 lg:h-80">
            <img src="/assets/${thumbnails[0]}" alt="${title}" class="h-full w-full object-cover object-center lg:h-full lg:w-full">
        </div>
        <div class="mt-4 flex justify-between">
            ...
            <button style="z-index: 999" type="button" onclick="return deleteProduct('${id}'); return false;">
